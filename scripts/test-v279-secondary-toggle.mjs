...
assert.ok(
  ...
  '合并面板 store 必须通过同步函数把新数组写回 rightPrompts',
);
assert.ok(source.includes('V2.79 已加载：下方预设开关改用不可变数组更新'), '缺少 v2.79 加载标记');

console.log('v2.79 下方面板开关测试通过：跨预设拖入后点击会立即替换数组与目标条目。');
