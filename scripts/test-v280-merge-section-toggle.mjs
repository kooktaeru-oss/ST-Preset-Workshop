...
assert.ok(helperStart >= 0 && helperEnd > helperStart, '无法定位缝合开关同步函数');
...
assert.ok(
  ...
  '缝合 store 必须同时更新 rightPrompts 与分组渲染状态',
);
assert.ok(source.includes('V2.80 已加载：缝合下方普通条目开关同步分组渲染状态'), '缺少 v2.80 加载标记');

console.log('v2.80 缝合开关测试通过：刚打开下方面板时，分组内外普通条目均同步渲染状态。');
