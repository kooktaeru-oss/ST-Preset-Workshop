...
assert.ok(
  source.includes('async function _pmmRevealPromptForCompare(e)'),
  'PresetPanel 缺少按名称揭示对比条目的方法',
);
assert.ok(
...
assert.ok(
  source.includes('reveal:_pmmRevealPromptForCompare'),
  'PresetPanel DOM 没有暴露对比揭示桥',
);
assert.ok(
  source.includes('compareRevealPrompt:_pmmRevealPromptForCompare'),
  'PresetPanel Vue 接口没有暴露对比揭示方法',
);
...
assert.ok(
  source.includes('V2.87 已加载：手机对比分组条目时会自动展开目标分组并读取正文。'),
  '缺少 v2.87 修复标记',
);

console.log('v2.87 分组对比测试通过：点击差异会自动展开两侧目标分组，再读取正文。');
