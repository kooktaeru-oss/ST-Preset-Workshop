...
mustInclude('pmm-wb-header-left${sideName === \'top\' ? \' header-left\' : \'\'}', '世界书标题栏没有区分上方固定外框与下方原结构');
mustInclude('<span class="pmm-wb-title-row">', '世界书标题栏缺少名称行');
mustInclude('header-card title-card title-card--interactive pmm-wb-title-card', '上方世界书没有复用预设名称外框');
mustInclude('<div class="pmm-wb-header-right">', '世界书标题栏缺少右侧工具栏区域');
mustInclude('.pmm-wb-header-left{flex:1 1 auto}', '世界书标题栏左侧没有弹性占用可用空间');
mustInclude('.pmm-wb-header-left>.pmm-wb-title-row,.pmm-wb-header-left>.pmm-wb-title-card,.pmm-wb-title-card>.pmm-wb-title-row{width:100%;min-width:0;flex:1 1 auto}', '世界书名称行和上方标题卡没有填满左侧区域');
