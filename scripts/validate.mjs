...
if (workshop.length < 1_000_000 || !workshop.includes('V2.63 已加载')) {
  throw new Error(`v2.63 业务入口不完整：${workshop.length} 字符`);
}

if (!entry.includes('workshop-v2.63.js') || !entry.includes("const EXTENSION_VERSION = '2.63.0'")) {
  throw new Error('扩展启动器没有指向 v2.63');
}

if (!workshop.includes('readPresetExtensionField?.({name:requested,path:PATH})')) {
  throw new Error('v2.63 没有按指定预设读取柏宝箱分组');
}

if (!workshop.includes('writePresetExtensionField({name:presetName,path:PATH')) {
  throw new Error('v2.63 没有按指定预设写入柏宝箱分组');
}

if (!workshop.includes("String(n).startsWith('branch:')") ||
    !workshop.includes('分支直接使用已保存的柏宝箱分组快照')) {
  throw new Error('v2.63 缺少分支快照读取隔离修复');
}
